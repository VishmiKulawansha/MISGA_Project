...
if (isset($_POST['Register'])) {
    ...
    if ($query->num_rows > 0) {
        ...
        while($record = $query->fetch_assoc()) {
            ...
            // Validate password
            if ($password == $record["farmerCode"]) {

                // Save the User Profile for later use
                $_SESSION['fullName'] = $record["fullName"];
                $_SESSION['farmerCode'] = $record["farmerCode"];
                $_SESSION['farmName'] = $record["farmName"];  
                $_SESSION['farmAddress'] = $record["farmAddress"];
                $_SESSION['farmArea'] = $record["farmArea"];
